...
describe('secondlevelController', function() {
    ...
    beforeEach(function(){
        $controller("SecondLevelController as vm", {
                $scope: scope
            });
    });
   ...
   describe('before activation', function() {
	   it('should have empty secondlevel array', function(){
	       expect(scope.vm.secondLevels).toEqual(jasmine.anything());
	   });
	
	   it('should have empty secondlevel array length', function(){   
	       expect(scope.vm.secondLevels.length).toEqual(0);
	   });   
   });   
     
   describe('after activation', function() {   
	   var mockSecondlevelsResponse = [];
	   var mockSecondlevelResponse = null;
	   
	    beforeEach(function(){
	    	
	    	mockSecondlevelsResponse =  [{"id": 1,
			...
			httpBackend.when('GET', 'api/secondlevels?max=-1').respond(mockSecondlevelsResponse);  // This is also valid httpBackend.whenGET('api/secondlevels?max=-1').respond(secondlevelsList); or  //httpBackend.whenGET(/secondlevels/).respond(secondlevelsList);
			httpBackend.when('GET', 'api/secondlevels/1?max=-1').respond(mockSecondlevelResponse);
			httpBackend.when('PUT', 'api/secondlevels/1?max=-1').respond(mockSecondlevelResponse); 
			httpBackend.when('POST', 'api/secondlevels?max=-1').respond(mockSecondlevelResponse); 
			httpBackend.when('DELETE', 'api/secondlevels/1?max=-1').respond(mockSecondlevelResponse); 

	    });
	    
	   it('should respond success to a GET', function(){  
	       httpBackend.expect('GET', 'api/secondlevels?max=-1').respond(200, 'success');
	   });  	   
		   
	    it('should have a function to refresh the secondlevels list', function(){
	        expect(scope.vm.list).toBeDefined();
	    });
	    
	    it('should have mocked mockSecondlevelsResponse', function(){
	    	expect(mockSecondlevelsResponse.length).toBeGreaterThan(0);
	    });
	   
	   it('should not have empty secondlevel array', function(){  
	       
	   	   scope.vm.list();
	   	   httpBackend.flush();
	       expect(scope.vm.secondLevels.length).toBeGreaterThan(0);
	   });    

	   xit('should have secondlevels list that matches mock', function(){  
		   
		   scope.vm.list();
		   httpBackend.flush();
		   expect(scope.vm.secondLevels).toEqual(mockSecondlevelsResponse);
 
	   }).pend('Because the types returned are not the same.  To do with promises.  Need to understand this better.'); 
	   
	   it('should have secondlevels list with secondlevels that matches mock', function(){  
		   
		   scope.vm.list();
		   httpBackend.flush();
		   expect(scope.vm.secondLevels[0].id).toEqual(1);
		   expect(scope.vm.secondLevels[0].nonGradeLevel).toEqual("Course -");
		   expect(scope.vm.secondLevels[0].nonGradeTitle).toEqual("United States History, Beginnings to 1877");
		   expect(scope.vm.secondLevels[0].title).toEqual("United States History, Beginnings to 1877");
		   expect(scope.vm.secondLevels[0].content[0].id).toEqual(9);
		   
	   }); 
	   
	   it('should add a secondlevel to the list', function(){  
		   scope.vm.newTitle = "A new title";
		   scope.vm.newHierarchy = 9;
		   scope.vm.addSecondLevel(mockSecondlevelResponse);
		   httpBackend.flush();
		   httpBackend.expect('POST', 'api/secondlevels?max=-1').respond(mockSecondlevelResponse); 
	   });
	   
  
	   it('should update a secondlevel title', function(){  
		   
		   scope.vm.updateTitle(mockSecondlevelResponse, 'A new title');
		   httpBackend.flush();
		   httpBackend.expect('PUT', 'api/secondlevels/1?max=-1').respond(mockSecondlevelResponse);
	   }); 
	
	   it('should update a secondlevel hierarchy', function(){  
		   
		   scope.vm.updateHierarchy(mockSecondlevelResponse, '7');
		   httpBackend.flush();
		   httpBackend.expect('PUT', 'api/secondlevels/1?max=-1').respond(mockSecondlevelResponse);
	   }); 
	   
	   it('should delete a secondlevel from the list', function(){  
		   
		   scope.vm.delete(mockSecondlevelResponse);
		   httpBackend.flush();  
		   httpBackend.expect('DELETE', 'api/secondlevels/1?max=-1').respond(mockSecondlevelResponse); 
